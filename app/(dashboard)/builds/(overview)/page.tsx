...
export default async function Page({
  ...
  const filteredBuilds = builds.filter((build) => {
    ...
    const buildActivities = build.activities.map(activity => {
      try {
    ...
    const activityMatch = !searchParams?.activity || 
    ...
    const subActivityMatch = !searchParams?.subactivity || 
      buildActivities.some(buildActivity => {
        const activityName = buildActivity.name.toLowerCase();
        return activityName === searchParams.subactivity?.toLowerCase();
      });
    
    const encounterMatch = !searchParams?.encounter || 
      buildActivities.some(buildActivity => {
        const activityName = buildActivity.name.toLowerCase();
        return activityName.includes(searchParams.encounter?.toLowerCase() || '');
      });
