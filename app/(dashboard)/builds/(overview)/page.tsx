...
export default async function Page({
  ...
  const filteredBuilds = builds.filter((build) => {
    ...
    if (activityTypeToMatch) {
      const activity = activities.find(activity => activity.id === activityTypeToMatch)
      if (activity) {
    ...
    if (subActivityToMatch) {
      const mainActivity = activities.find(activity => activity.id === resolvedSearchParams?.activity)
      if (mainActivity) {
        const subActivity = mainActivity.subActivities?.find(subActivity => subActivity.id === subActivityToMatch)
        if (subActivity) {
    ...
    if (encounterToMatch) {
      const mainActivity = activities.find(activity => activity.id === resolvedSearchParams?.activity)
      if (mainActivity) {
        const subActivity = mainActivity.subActivities?.find(subActivity => subActivity.id === resolvedSearchParams?.subactivity)
        if (subActivity) {
          const encounter = subActivity.subActivities?.find(e => e.id === encounterToMatch)
          if (encounter) {
    ...
    const encounterMatch =
      ...
      buildActivities.some((buildActivity) => {
        ...
        return activityName === searchEncounter ||
