...
export default async function BuildPage({ params }: { params: Params }) {
  ...
  const legendaryWeapons = build.legendary_weapons
    ...
    : []
  const superAbility = build.super_ability?.[0] ? JSON.parse(build.super_ability[0]) : null
    ...
    <div className="flex flex-col md:flex-row gap-8 p-6">
      ...
      <div className="md:w-2/3 space-y-8">
        ...
        <div className="bg-primary-dark rounded-lg p-6">
          <h2 className="text-sm font-medium text-gray-400 mb-4">Recommended Activities</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {build.activities.map((activity: string, index: number) => {
              ...
              return (
                <div key={index} className="flex items-center gap-3">
                  ...
                  <span className="text-white text-sm truncate">{parsedActivity.name}</span>
                </div>
              )
            })}
          </div>
        ...
        <div className="bg-primary-dark rounded-lg p-6">        
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {superAbility && (
              <div>
                <h3 className="text-sm font-medium text-gray-400 mb-4">Super Ability</h3>
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-white text-sm">{superAbility.name}</span>
                </div>
              </div>
            )}
            ...
            {aspects.length > 0 && (
              <div>
                <h3 className="text-sm font-medium text-gray-400 mb-4">Aspects</h3>
                <div className="space-y-3">
                  {aspects.map((aspect: Aspect, index: number) => (
                    <div key={index} className="flex items-center gap-4">
                      ...
                      <span className="text-white text-sm">{aspect.name}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {fragments.length > 0 && (
              <div>
                <h3 className="text-sm font-medium text-gray-400 mb-4">Fragments</h3>
                <div className="space-y-3">
        ...
        <div className="bg-primary-dark rounded-lg p-6">        
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {exoticArmor && (
              <div>
                <h3 className="text-sm font-medium text-gray-400 mb-4">Exotic Armor</h3>
                <div className="flex items-center gap-4">
                  <div className="relative w-12 h-12 rounded-full overflow-hidden bg-primary-dark/50">
                    <Image
                  ...
                  <span className="text-white text-sm">{exoticArmor.name}</span>
                </div>
              </div>
            )}
            ...
            {exoticWeapon && (
              <div>
                <h3 className="text-sm font-medium text-gray-400 mb-4">Exotic Weapon</h3>
                <div className="flex items-center gap-4">
                  <div className="relative w-12 h-12 rounded-full overflow-hidden bg-primary-dark/50">
                    <Image
                  ...
                  <span className="text-white text-sm">{exoticWeapon.name}</span>
                </div>
              </div>
            )}
          </div>
