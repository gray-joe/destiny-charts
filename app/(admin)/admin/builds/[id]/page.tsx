...
export default async function EditBuildPage({ params }: { params: Params }) {
    ...
    const currentExoticArmor = build.exotic_armor?.[0]
        ? JSON.parse(build.exotic_armor[0])
        : null
    const currentExoticWeapon = build.exotic_weapon?.[0]
        ? JSON.parse(build.exotic_weapon[0])
        : null
    const currentSuperAbility = build.super_ability?.[0]
                    ...
                    <div className="flex-grow">
                        <form
                            action={async (formData: FormData) => {
                                ...
                                const class_name = formData.get(
                                    'class'
                                ) as string
                                const subclass_name = formData.get(
                                    'subclass'
                                ) as string
                                const exotic_armor_id = formData.get(
                                    'exotic_armor'
                                ) as string
                                const exotic_weapon_id = formData.get(
                                    'exotic_weapon'
                                ) as string
                                const super_ability_id = formData.get(
                                    'super_ability'
                                ) as string
                                const activity_ids = formData.getAll(
                                    'activities'
                                ) as string[]
                                const aspect_ids = formData.getAll(
                                    'aspects'
                                ) as string[]
                                const fragment_ids = formData.getAll(
                                    'fragments'
                                ) as string[]
                                            ...
                                            {activities.map((activity) => (
                                                ...
                                                >
                                                    <input
                                                        ...
                                                        name="activities"
                                                        value={activity.id}
                                                        defaultChecked={currentActivities.some(
                                                            (a: Activity) =>
                                                                a.name ===
                                                                activity.name
                                                        )}
                                            ...
                                            {aspects.map((aspect) => (
                                                ...
                                                >
                                                    <input
                                                        ...
                                                        name="aspects"
                                                        value={aspect.id}
                                                        defaultChecked={currentAspects.some(
                                                            (a: Aspect) =>
                                                                a.name ===
                                                                aspect.name
                                                        )}
                                            ...
                                            {fragments.map((fragment) => (
                                                ...
                                                >
                                                    <input
                                                        ...
                                                        name="fragments"
                                                        value={fragment.id}
                                                        defaultChecked={currentFragments.some(
                                                            (f: Fragment) =>
                                                                f.name ===
                                                                fragment.name
                                                        )}
