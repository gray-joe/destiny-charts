...
export default function AbilitiesTable({
  ...
  const columns = [
    ...
    { key: 'subtype', label: 'Subtype', always: true, className: 'min-w-[120px]' },
    { key: 'modifiers', label: 'Modifiers', always: true, className: 'min-w-[150px] max-w-[300px] break-words' },
    { key: 'count', label: 'Count', always: false, className: 'min-w-[80px]' },
    { key: 'percentage', label: 'Percentage', always: false, className: 'min-w-[100px]' },
    { key: 'actual', label: 'Actual', always: true, className: 'min-w-[100px]' },
    { key: 'wipe', label: 'Wipe', always: false, className: 'min-w-[100px]' },
    { key: 'ratio', label: 'Ratio', always: false, className: 'min-w-[100px]' },
    { key: 'deviation', label: 'Deviation', always: false, className: 'min-w-[100px]' },
    { key: 'updated_at', label: 'Last Updated', always: false, className: 'min-w-[150px]' },
  ];
                    ...
                    <tr key={item.id} className="group hover:bg-primary-hover">
                      {columns.map(column => {
                        ...
                        if (column.key === 'icon_url') {
                          return (
                            <td key={column.key} className={`px-3 py-5 ${column.className}`}>
                              {value ? (
                                <Image
                                  src={value as string}
                                  alt={item.type}
                        ...
                        return (
                          <td 
                            ...
                            className={`px-3 py-5 text-sm ${column.className} ${
                              column.key === 'actual' ? getActualColor(value as string) : ''
                            }`}
                          >
