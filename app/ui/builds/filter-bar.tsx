...
import { MainActivity, SubActivity, EncounterActivity } from '@/app/lib/activity-data';

export default function FilterBar({ activities }: { activities: MainActivity[] }) {
  const router = useRouter();
  const searchParams = useSearchParams();
  
  const [selectedActivity, setSelectedActivity] = useState(searchParams.get('activity') || '');
  const [selectedSubActivity, setSelectedSubActivity] = useState(searchParams.get('subactivity') || '');
  const [selectedEncounter, setSelectedEncounter] = useState(searchParams.get('encounter') || '');

  // Find the currently selected activity and subactivity objects
  const currentActivity = activities.find(a => a.id === selectedActivity) as MainActivity | undefined;
  const currentSubActivity = currentActivity?.subActivities?.find(sa => sa.id === selectedSubActivity) as SubActivity | undefined;
